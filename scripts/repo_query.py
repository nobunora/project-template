...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("paths", nargs="*", default=["src", "include", "tests"])
    ...
    sp = sub.add_parser("symbol")
    sp.add_argument("name")
    sp.add_argument("--max-results", type=int, default=100)
    for name in ("refs", "callers", "callees"):
        sp = sub.add_parser(name)
        sp.add_argument("target", help="symbol name or path:line[:column]")
    sp = sub.add_parser("tests")
    sp.add_argument("pattern")
    sp.add_argument("--max-results", type=int, default=200)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if cmd == "index":
            result = build_ctags_index(root, index_path, args.paths)
        elif cmd == "files":
            result = list_files(root, args.pattern)
        elif cmd == "text":
            result = rg_json(root, args.pattern, max_results=args.max_results)
        elif cmd == "symbol":
            result = symbol_query(root, index_path, args.name, args.max_results)
        elif cmd in ("refs", "callers", "callees"):
            db = find_compile_db(root, args.compile_db)
            if not shutil.which("clangd"):
                raise ToolError("clangd not found; semantic query unavailable")
            if not db:
                raise ToolError("compile_commands.json not found; semantic C/C++ query unavailable")
            result = semantic_query(root, db, index_path, cmd, args.target)
        elif cmd == "tests":
            result = tests_query(root, args.pattern, args.max_results)
        elif cmd == "changed":
            result = changed_files(root)
        elif cmd == "doctor":
            result = doctor(root, args.compile_db, index_path)
        else:
            raise ToolError(f"unknown command: {cmd}")
