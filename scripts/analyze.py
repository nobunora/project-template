...
def find_compile_db(root: Path, explicit: str | None) -> Path | None:
    if explicit:
        ...
        if p.is_dir(): p = p / "compile_commands.json"
    ...
    for p in root.glob("**/compile_commands.json"):
        if not any(part in {".git", "node_modules", ".venv"} for part in p.parts): return p.resolve()
    return None
...
def normalize_db_files(root: Path, db: list[dict[str, Any]]) -> dict[str, Path]:
    ...
    for item in db:
        ...
        p = p.resolve()
        if p.suffix.lower() in CPP_SUFFIXES: out[rel(root, p)] = p
    return out
...
def changed_files(root: Path) -> set[str]:
    ...
    for argv in (["git", "diff", "--name-only"], ["git", "diff", "--cached", "--name-only"],
        ...
        if cp.returncode == 0: names.update(x.replace("\\", "/") for x in cp.stdout.splitlines() if x)
    return names
...
def select_translation_units(root: Path, db_files: dict[str, Path], profile: str,
    ...
    if profile == "deep":
        ...
        if not explicit_file: raise ToolError("file profile requires a path")
...
def clang_tidy_one(root: Path, db_path: Path, source: Path, checks: str | None, raw_dir: Path) -> dict[str, Any]:
    ...
    if checks: argv += [f"-checks={checks}"]
    cp = run(argv, cwd=root)
...
def load_baseline(root: Path, baseline_path: str | None) -> tuple[set[str], str | None]:
    if not baseline_path: return set(), None
    p = Path(baseline_path)
    if not p.is_absolute(): p = root / p
    if not p.exists(): return set(), rel(root, p)
    try: data = json.loads(p.read_text(encoding="utf-8"))
    except Exception as exc: raise ToolError(f"invalid baseline file {rel(root, p)}: {exc}") from exc
    ids = data.get("finding_ids", []) if isinstance(data, dict) else data if isinstance(data, list) else None
    if ids is None: raise ToolError("baseline must be a list or an object with finding_ids")
...
def should_fail(findings: list[dict[str, Any]], fail_on: str) -> bool:
    ...
    if fail_on == "error": return any(f["level"] == "error" for f in new)
    if fail_on == "high": return any(f["severity"] in {"critical", "high"} for f in new)
    if fail_on == "any": return bool(new)
...
def analyze(root: Path, db_path: Path, profile: str, explicit_file: str | None, jobs: int,
    ...
    if not db_files: raise ToolError("no C/C++ translation units found in compile_commands.json")
    ...
    if not selected:
        ...
        write_json(output_dir / "findings.json", []); write_json(output_dir / "summary.json", summary)
        return summary, 0
    ...
    for result in results:
        for finding in result["findings"]:
            ...
            seen.add(obj["id"]); findings_obj.append(obj)
    findings_obj.sort(key=lambda x: (x["path"], x["line"], x["column"], x["check"], x["message"]))
    write_json(output_dir / "findings.json", findings_obj)
    counts = {"critical": 0, "high": 0, "medium": 0, "low": 0}
    for f in findings_obj: counts[f["severity"]] = counts.get(f["severity"], 0) + 1
    new_count = sum(1 for f in findings_obj if f["new_in_analysis"])
    if crashed: status, exit_code = "error", 3
    elif should_fail(findings_obj, fail_on): status, exit_code = "fail", 1
    else: status, exit_code = "pass", 0
...
def doctor(root: Path, db_arg: str | None) -> tuple[dict[str, Any], int]:
    ...
    clang_tidy = shutil.which("clang-tidy"); add("clang-tidy", clang_tidy is not None, clang_tidy or "")
    ...
    if db_path:
        ...
        except ToolError as exc: add("compile_commands", False, str(exc))
    else: add("compile_commands", False, "not found")
    config = root / ".clang-tidy"; add(".clang-tidy", config.exists(), rel(root, config), required=False)
...
def create_baseline(root: Path, source_path: str, output_path: str) -> dict[str, Any]:
    ...
    if not source.is_file(): raise ToolError(f"findings file not found: {rel(root, source)}")
    try: data = json.loads(source.read_text(encoding="utf-8"))
    except Exception as exc: raise ToolError(f"cannot parse findings file: {exc}") from exc
    if not isinstance(data, list): raise ToolError("findings file must contain a JSON array")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("fast"); sub.add_parser("normal"); sub.add_parser("deep")
    sp = sub.add_parser("file"); sp.add_argument("path")
    sub.add_parser("doctor")
    sp = sub.add_parser("baseline"); sp.add_argument("--from-findings", required=True, dest="from_findings"); sp.add_argument("--output", required=True)
    return p


def emit(obj: Any, compact: bool) -> None:
    json.dump(obj, sys.stdout, ensure_ascii=False, indent=None if compact else 2); sys.stdout.write("\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "doctor":
            result, code = doctor(root, args.compile_db); emit(result, args.compact); return code
        if args.command == "baseline":
            result = create_baseline(root, args.from_findings, args.output); emit(result, args.compact); return 0
        db_path = find_compile_db(root, args.compile_db)
        if not db_path: raise ToolError("compile_commands.json not found")
        if not shutil.which("clang-tidy"): raise ToolError("clang-tidy not found")
        ...
        emit(summary, args.compact); return code
    except ToolError as exc:
        emit({"status": "error", "error": str(exc)}, getattr(args, "compact", False)); return 2
